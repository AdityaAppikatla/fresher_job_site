        ...
        try:
            ...
            if resp.status_code == 200:
                for job in resp.json().get("results", []):
                    ...
                    # --- CRITICAL FIX: BYPASS ADZUNA REGISTRATION WALLS ---
                    # Instead of grabbing Adzuna's default redirect track link, we try to grab
                    # the underlying redirect target or clean up tracking wrappers via string processing
                    raw_url = job.get("redirect_url", "")
                    if not raw_url:
                        continue
                        
                    clean_url = raw_url
                    # Look inside Adzuna tracking parameters to find embedded origin parameters if they exist
...
def main():
    ...
    print(f"Deployment complete. Upgraded direct application pathways.")
